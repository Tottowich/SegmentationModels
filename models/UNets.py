...
class UNetEncoder(nn.Module):
    ...
    """
    def __init__(self, config:Union[dict,str],in_channels:int=3):
        super().__init__()
...
class DecoderBlock(nn.Module):
    def __init__(self,
        ...
        if attention:
            ...
            self.skip = Conv1x1(out_channels,out_channels)
        self.attention = attention
    ...
    def forward(self,x_in,context):
        ...
        # print(f"DecoderBlock x post conv_attn: {x.shape}")s
        if self.attention:
        ...
        # print(f"DecoderBlock x post upsample: {x.shape}")
    ...
    def build_decoder(self):
        ...
        for i in range(self.steps):
            decoder.append(DecoderBlock(
                ...
                out_channels=self.decoder_config["out_channels"][i],
                img_size=img_size,
                context_channels=self.decoder_config["context_channels"][i],
...
class UNet(nn.Module):
    def __init__(self,config,device:T.device=T.device("cpu"),in_channels=3,verbose=False):
        super().__init__()
        self.device = device
        self.encoder = UNetEncoder(config,in_channels=in_channels)
        self.decoder = UNetDecoder(config,self.encoder.latent_size)
